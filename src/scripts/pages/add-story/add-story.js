...
const AddStory = {
  ...
  // ✅ Fungsi kirim notifikasi lokal (Perbaikan)
  async sendLocalNotification(title, body) {
    try {
      ...
      await registration.showNotification(title, {
        ...
        vibrate: [200, 100, 200],
        tag: `story-added-${Date.now()}`, // Unik setiap kali
        renotify: true, // Force notify user
        ...
        actions: [
          { action: "open", title: "Lihat", icon: "/icons/icon-192x192.png" },
          { action: "close", title: "Tutup" },
        ],
      });

      console.log("[Notification] Local notification shown");
    } catch (error) {
